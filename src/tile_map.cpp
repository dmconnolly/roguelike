...
{
	static const uint64_t max_cost = static_cast<uint64_t>(std::numeric_limits<uint64_t>::max());
	...
	while(!open_set.empty()) {
		Tile &current = **std::min_element(open_set.begin(), open_set.end(), [f_cost](Tile *a, Tile *b) {
			const uint64_t a_cost = get_with_default(f_cost, a, max_cost);
			const uint64_t b_cost = get_with_default(f_cost, b, max_cost);
		...
		for(const auto direction : neighbour_directions) {
			...
			const uint64_t tmp_g_cost = 1 + get_with_default(g_cost, &current, max_cost);
			const uint64_t neighbour_g_cost = get_with_default(g_cost, &neighbour, max_cost);
